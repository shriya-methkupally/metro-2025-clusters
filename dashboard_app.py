...
<style>
  ...
  /* center all tables and force black text */
...
group_defs = {
    'AI Superstars':       "...",  # your definitions
    'Star AI Hubs':        "...",
    'Emerging AI Centers': "...",
    'Focused AI Scalers':  "...",
    'Nascent AI Adopters': "...",
...
if mode == "Group Overviews":
    ...
    cols = st.columns(5)
    stats = [('Count',cnt),('Mean',row['Mean']),('Min',row['Min']),('Max',row['Max']),('Range',row['Range'])]
...
# ─── Group Comparison ─────────────────────────────────────────────────────────
    ...
    if not selected:
        ...
        rows = []
        # now include counts, shares for count_metrics, and MEANS for firm, talent, innovation
        for m in count_metrics + firm_metrics + talent_metrics + innovation_metrics:
            if m in count_metrics:
                total_sel = summary_df[(summary_df['Group'].isin(selected)) & (summary_df['Metric']==m)]['Sum'].sum()
